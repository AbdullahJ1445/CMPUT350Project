...
void Strategy::loadStrategies() {
	...
	if (map_index == 1) {
		...
		if (p_id == 1) {
			bot->storeLocation("PYLON_1", sc2::Point2D(68, 155));
			bot->storeLocation("FORGE_1", sc2::Point2D(68.5, 148.5));
			bot->storeLocation("PYLON_2", sc2::Point2D(68.0, 153.0));
		...
		if (p_id == 2) {
			bot->storeLocation("PYLON_1", sc2::Point2D(155,124));
			bot->storeLocation("FORGE_1", sc2::Point2D(148.5,123.5));
			bot->storeLocation("PYLON_2", sc2::Point2D(153,124));
		...
		if (p_id == 3) {
			bot->storeLocation("PYLON_1", sc2::Point2D(124,37));
			bot->storeLocation("FORGE_1", sc2::Point2D(123.5, 43.5));
			bot->storeLocation("PYLON_2", sc2::Point2D(124.5,39));
		...
		if (p_id == 4) {
			bot->storeLocation("PYLON_1", sc2::Point2D(37,68));
			bot->storeLocation("FORGE_1", sc2::Point2D(43.5,68.5));
			bot->storeLocation("PYLON_2", sc2::Point2D(39,68));
		...
		}
		{
			...
			t.add_condition(COND::MAX_UNIT_OF_TYPE_NEAR_LOCATION, 0, sc2::UNIT_TYPEID::PROTOSS_FORGE, bot->getStoredLocation("FORGE_1"), 0.5F);
			forge_1.addDirective(d);
			forge_1.addTrigger(t);
			Trigger t2(bot);
			t2.add_condition(COND::MIN_MINERALS, 150);
			t2.add_condition(COND::MIN_UNIT_OF_TYPE_NEAR_LOCATION, 1, sc2::UNIT_TYPEID::PROTOSS_NEXUS, bot->locH->bases[1].get_townhall());
			t2.add_condition(COND::MAX_UNIT_OF_TYPE_NEAR_LOCATION, 0, sc2::UNIT_TYPEID::PROTOSS_PYLON, bot->getStoredLocation("FORGE_1"), 0.5F);
